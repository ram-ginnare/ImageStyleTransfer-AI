...
# # from utils.utlis import ImageFolderDataset
...
content_data=str(BASE_DIR / "content_data")
style_data=str(BASE_DIR / "style_data")
vgg_path=str(BASE_DIR / "vgg_normalised.pth")

def parse_arguments():
    parser = argparse.ArgumentParser()

    parser.add_argument('--content_dir', type=str, default=content_data,
                         help='Location of content images')
    parser.add_argument('--style_dir', type=str, default=style_data,
                         help='Location of style images')
    ...
    parser.add_argument("--experiment", type=str, default=str(BASE_DIR / "experiment1"),
...
def main():
    ...
    print(f"Content directory: {args.experiment}")
    ...
    print('Training...')
    if args.resume:
        decoder.load_state_dict(torch.load(save_dir / args.decoder_path))
        optimizer.load_state_dict(torch.load(save_dir / args.optimizer_path))
        print('Resumed from checkpoint.')

    nes_loss = torch.nn.MSELoss()
    ...
    for epoch in range(args.epochs):
        ...
        running_closs = 0
        running_sloss = 0
        for i, (content_batch, style_batch) in enumerate(progress_bar):
            ...
            # print('--------------------')
            # print(len(c_feats), len(s_feats))
            # print(c_feats[0].shape, s_feats[0].shape)
            t = adaptive_instance_normalization(c_feats[-1], s_feats[-1])
            g = decoder(t)
            g_feats = encoder(g)

            loss_c = nes_loss(g_feats[-1], t) * args.content_weight
            loss_s = 0
            for g_f, s_f in zip(g_feats, s_feats):
                g_mean, g_std = calc_mean_std(g_f)
                s_mean, s_std = calc_mean_std(s_f)
                loss_s += nes_loss(g_mean, s_mean) + nes_loss(g_std, s_std)
