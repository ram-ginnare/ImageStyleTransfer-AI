...
print("Loading pre-trained decoder weights..."+str(BASE_DIR / 'experiment' / 'final_exp' / 'decoder_final.pth'))
decoder.load_state_dict(torch.load(str(BASE_DIR / 'experiment' / 'final_exp' / 'decoder_final.pth'),
                                    map_location=device))
...
def save_image(image, path):
    image = image.cpu().clone() # clone the tensor to avoid modifying the original
    image = image.squeeze(0)   # remove the batch dimension
    image = image.clamp(0, 1)  # clamp the values to [0, 1]
    image = transforms.ToPILImage()(image) # convert the tensor to a PIL image
